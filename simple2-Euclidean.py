...
with open("./facebook_test") as fr:
    for row in fr:
        ...
        #results.append(np.linalg.norm(features[id1]-features[id2]))

        x1=norm(features[id1])
        x2=norm(features[id2])
        scale = max(x1,x2)
        mini = min(x1,x2)
        #res = np.sum((features[id1] / scale) * (features[id2] / scale))

        #res = np.sum(features[id1]* features[id2])

        res =np.linalg.norm(features[id1]/scale-features[id2]/scale)
...
with open("./fbEuclidean2.csv", 'w') as fw:
